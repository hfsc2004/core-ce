/**
 * Pseudo Science Fiction Core Collection - Installation Manager macOS ARM (Apple Silicon)
...
async function buildPythonWebUI(fromPath, outputCallback = null) {
  try {
    ...
    try {
      ...
      console.warn('[Installation Manager macOS ARM (Apple Silicon)] Could not chmod build script:', e);
    }
    ...
    return new Promise((resolve) => {
      ...
      buildProcess.stdout?.on('data', (data) => {
        ...
        console.log('[Installation Manager macOS ARM (Apple Silicon)] [Python WebUI]', line.trim());
      });
...
async function installAnythingLLM(fromPath, outputCallback = null) {
  try {
    ...
    console.log('[Installation Manager macOS ARM (Apple Silicon)] Starting AnythingLLM installation...');
    
    let output = '';
    const sendOutput = (line) => {
      output += line + '\n';
      if (outputCallback) outputCallback(line);
      console.log('[Installation Manager macOS ARM (Apple Silicon)] [AnythingLLM]', line);
    };
...
async function killAnythingLLMProcesses() {
  console.log('[Installation Manager macOS ARM (Apple Silicon)] ðŸ”ª Killing AnythingLLM processes...');
  
  try {
    await execPromise('pkill -9 -f "anything-llm" 2>/dev/null || true');
    await execPromise('pkill -9 -f "node.*server" 2>/dev/null || true');
    console.log('[Installation Manager macOS ARM (Apple Silicon)] ✅ Killed AnythingLLM processes');
    return { success: true };
  } catch (e) {
    console.log('[Installation Manager macOS ARM (Apple Silicon)] ℹ️  No processes found');
    return { success: true };
  }
}
