/**
 * Pseudo Science Fiction Core Collection - Installation Manager Linux x64
...
async function buildPythonWebUI(fromPath, outputCallback = null) {
  try {
    ...
    try {
      ...
      console.warn('[Installation Manager Linux x64] Could not chmod build script:', e);
    }
    ...
    return new Promise((resolve) => {
      ...
      buildProcess.stdout?.on('data', (data) => {
        ...
        console.log('[Installation Manager Linux x64] [Python WebUI]', line.trim());
      });
...
async function installAnythingLLM(fromPath, outputCallback = null) {
  try {
    ...
    console.log('[Installation Manager Linux x64] Starting AnythingLLM installation...');
    
    let output = '';
    const sendOutput = (line) => {
      output += line + '\n';
      if (outputCallback) outputCallback(line);
      console.log('[Installation Manager Linux x64] [AnythingLLM]', line);
    };
...
async function killAnythingLLMProcesses() {
  console.log('[Installation Manager Linux x64] ðŸ”ª Killing AnythingLLM processes...');
  
  try {
    await execPromise('pkill -9 -f "anything-llm" 2>/dev/null || true');
    await execPromise('pkill -9 -f "node.*server" 2>/dev/null || true');
    console.log('[Installation Manager Linux x64] ✅ Killed AnythingLLM processes');
    return { success: true };
  } catch (e) {
    console.log('[Installation Manager Linux x64] ℹ️  No processes found');
    return { success: true };
  }
}
