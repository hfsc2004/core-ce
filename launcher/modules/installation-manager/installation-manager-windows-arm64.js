  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      buildProcess.stdout?.on('data', (data) => {
        ...
        console.log('[Installation Manager Windows ARM64] [Python WebUI]', line.trim());
      });
...
async function installAnythingLLM(fromPath, outputCallback = null) {
  try {
    ...
    const sendOutput = (line) => {
      output += line + '\n';
      if (outputCallback) outputCallback(line);
      console.log('[Installation Manager Windows ARM64] [AnythingLLM]', line);
    };
...
async function killAnythingLLMProcesses() {
  console.log('[Installation Manager Windows ARM64] 🔪 Killing AnythingLLM processes...');
  try {
    await execPromise('taskkill /F /IM node.exe /FI "WINDOWTITLE eq anything-llm*" 2>nul');
    console.log('[Installation Manager Windows ARM64] ✅ Killed processes');
    return { success: true };
  } catch (e) {
    console.log('[Installation Manager Windows ARM64] ℹ️  No processes found');
    return { success: true };
  }
}
