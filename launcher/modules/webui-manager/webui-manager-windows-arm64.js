...
async function killAllOllama() {
  console.log('[WebUI Windows ARM64] Checking for existing Ollama processes...');
  
  try {
    await execPromise('taskkill /F /IM ollama.exe /T');
    console.log('[WebUI Windows ARM64] Killed existing Ollama processes');
  } catch (e) {
    console.log('[WebUI Windows ARM64] No existing Ollama processes found');
  }
}
...
function forceKillProcess(proc, name) {
  if (proc && !proc.killed) {
    try {
      proc.kill();
      console.log(`[WebUI Windows ARM64] Killed ${name}`);
    } catch (err) {
      console.error(`[WebUI Windows ARM64] Error killing ${name}:`, err);
    }
  }
}
...
async function ensurePythonVenv(platformDir, dirname) {
  ...
  if (fs.existsSync(pythonExe)) {
    console.log('[WebUI Windows ARM64] Python WebUI environment found');
    return true;
  }
  
  console.log('[WebUI Windows ARM64] Python WebUI environment not found, building...');
  
  const buildScript = path.join(dirname, 'build-python-webui.bat');
  
  if (!fs.existsSync(buildScript)) {
    console.error('[WebUI Windows ARM64] Build script not found:', buildScript);
    return false;
  }
  
  return new Promise((resolve) => {
    console.log('[WebUI Windows ARM64] Building Python WebUI environment...');
    ...
    buildProcess.on('close', (code) => {
      if (code === 0) {
        console.log('[WebUI Windows ARM64] Python WebUI built successfully');
        resolve(true);
      } else {
        console.error(`[WebUI Windows ARM64] Build failed with code ${code}`);
        resolve(false);
      }
    });
    
    buildProcess.on('error', (err) => {
      console.error('[WebUI Windows ARM64] Build error:', err);
      resolve(false);
    });
  });
}
...
async function launchOpenWebUI(dirname, gpuInfo = null) {
  ...
  debugLog('[WebUI Windows ARM64] ================================================');
  debugLog('[WebUI Windows ARM64] LAUNCH OPEN WEBUI CALLED');
  debugLog('[WebUI Windows ARM64] ================================================');
  ...
  try {
    ...
    debugLog('[WebUI Windows ARM64] Starting PSF Ollama via BMOC...');
    ...
    try {
      ...
      if (!ollamaResult.success) {
        console.error('[WebUI Windows ARM64] Failed to start PSF Ollama:', ollamaResult.message);
        closeLoadingWindow();
      ...
      console.log(`[WebUI Windows ARM64] PSF Ollama started on port ${ollamaPort}`);
      debugLog(`[WebUI Windows ARM64] PSF Ollama running on port ${ollamaPort} (Session: ${currentSessionId})`);
    } catch (ollamaErr) {
      console.error('[WebUI Windows ARM64] Failed to start PSF Ollama:', ollamaErr);
      closeLoadingWindow();
    ...
    if (!ollamaReady) {
      console.warn('[WebUI Windows ARM64] Ollama may not be responding yet, but continuing...');
    } else {
      console.log(`[WebUI Windows ARM64] Ollama is responding on port ${ollamaPort}`);
    }
    
    updateLoadingStatus('Loading Open WebUI...', 'Preparing interface');
    
    const webuiDir = path.join(dirname, '..', 'binaries', 'python-webui');
    const platformDir = 'windows-arm64';
    
    console.log('[WebUI Windows ARM64] Platform: windows-arm64');
    ...
    if (openWebuiProcess && !openWebuiProcess.killed) {
      console.log('[WebUI Windows ARM64] Open WebUI already running');
    ...
    if (!webuiPort) {
      console.error('[WebUI Windows ARM64] Failed to allocate WebUI port');
      closeLoadingWindow();
      return { success: false, message: 'Failed to allocate port for Open WebUI' };
    }
    console.log(`[WebUI Windows ARM64] Allocated WebUI port: ${webuiPort}`);
    
    console.log('[WebUI Windows ARM64] Launching Open WebUI...');
    ...
    openWebuiProcess.on('exit', (code) => {
      console.log(`[WebUI Windows ARM64] Open WebUI exited with code ${code}`);
      openWebuiProcess = null;
    });
    
    console.log('[WebUI Windows ARM64] Open WebUI process started');
    
    console.log('[WebUI Windows ARM64] Waiting for Open WebUI to start...');
    ...
    for (let i = 0; i < maxWaitSeconds; i++) {
      ...
      if (checkReady) {
        webuiReady = true;
        console.log(`[WebUI Windows ARM64] Open WebUI ready after ${i + 1} seconds`);
        break;
      }
      
      if ((i + 1) % 5 === 0) {
        console.log(`[WebUI Windows ARM64] Still waiting... (${i + 1}/${maxWaitSeconds})`);
      }
    }
    
    if (!webuiReady) {
      console.error('[WebUI Windows ARM64] Open WebUI did not start in time');
      closeLoadingWindow();
      return { success: false, message: 'Open WebUI failed to start.' };
    }
    
    console.log('[WebUI Windows ARM64] Waiting for Open WebUI to connect to Ollama...');
    ...
    for (let i = 0; i < maxOllamaWait; i++) {
      ...
      if (checkOllama) {
        ...
        console.log(`[WebUI Windows ARM64] Open WebUI connected to Ollama after ${i + 1} seconds`);
        break;
      }
      
      if ((i + 1) % 3 === 0) {
        console.log(`[WebUI Windows ARM64] Still connecting... (${i + 1}/${maxOllamaWait})`);
      }
    }
    
    if (!ollamaConnected) {
      console.warn('[WebUI Windows ARM64] May not be fully connected, but opening anyway...');
    }
    ...
    // BMOC: Update session with WebUI process info
    ...
    console.log(`[WebUI Windows ARM64] Updated session: ${currentSessionId}`);
    
    // BMOC: Simplified cleanup
    openWebuiWindow.on('closed', () => {
      console.log('[WebUI Windows ARM64] Window closed, stopping backend...');
      
      forceKillProcess(openWebuiProcess, 'Open WebUI');
      openWebuiProcess = null;
      
      if (webuiPort) {
        console.log(`[WebUI Windows ARM64] Releasing WebUI port ${webuiPort}...`);
    ...
    console.error('[WebUI Windows ARM64] Launch error:', err);
