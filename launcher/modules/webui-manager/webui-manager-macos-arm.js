/**
 * Pseudo Science Fiction Core Collection - WebUI Manager macOS ARM (Apple Silicon)
 * Platform-specific implementation for macOS ARM (Apple Silicon) systems
...
async function killAllOllama() {
  console.log('[WebUI macOS ARM] 🪓 Checking for existing Ollama processes...');
  
  try {
    await execPromise('pkill -9 ollama || sudo -n pkill -9 ollama 2>/dev/null || true');
    console.log('[WebUI macOS ARM] ✅ Killed existing Ollama processes');
  } catch (e) {
    console.log('[WebUI macOS ARM] ℹ️  No existing Ollama processes found');
  }
}
...
function forceKillProcess(proc, name) {
  if (proc && !proc.killed) {
    try {
      ...
      console.log(`[WebUI macOS ARM] Killed ${name}`);
    } catch (err) {
      console.error(`[WebUI macOS ARM] Error killing ${name}:`, err);
    }
  }
}
...
async function ensurePythonVenv(platformDir, dirname) {
  ...
  if (fs.existsSync(pythonExe)) {
    console.log('[WebUI macOS ARM] ✅ Python WebUI environment found');
    return true;
  }
  
  console.log('[WebUI macOS ARM] ⚠️  Python WebUI environment not found, building...');
  
  const buildScript = path.join(dirname, 'build-python-webui-macos.sh');
  
  if (!fs.existsSync(buildScript)) {
    console.error('[WebUI macOS ARM] ❌ Build script not found:', buildScript);
    return false;
  }
  
  return new Promise((resolve) => {
    console.log('[WebUI macOS ARM] 🔨 Building Python WebUI environment...');
    ...
    buildProcess.on('close', (code) => {
      if (code === 0) {
        console.log('[WebUI macOS ARM] ✅ Python WebUI built successfully');
        resolve(true);
      } else {
        console.error(`[WebUI macOS ARM] ❌ Build failed with code ${code}`);
        resolve(false);
      }
    });
    
    buildProcess.on('error', (err) => {
      console.error('[WebUI macOS ARM] ❌ Build error:', err);
      resolve(false);
    });
  });
}
...
async function launchOpenWebUI(dirname, gpuInfo = null) {
  ...
  debugLog('[WebUI macOS ARM] ================================================');
  debugLog('[WebUI macOS ARM] 🚀 LAUNCH OPEN WEBUI CALLED');
  debugLog('[WebUI macOS ARM] ================================================');
  ...
  try {
    ...
    debugLog('[WebUI macOS ARM] 🏴‍☠️ Starting PSF Ollama via BMOC...');
    ...
    try {
      ...
      if (!ollamaResult.success) {
        console.error('[WebUI macOS ARM] Failed to start PSF Ollama:', ollamaResult.message);
        closeLoadingWindow();
      ...
      console.log(`[WebUI macOS ARM] ✅ PSF Ollama started on port ${ollamaPort}`);
      debugLog(`[WebUI macOS ARM] ✅ PSF Ollama running on port ${ollamaPort} (Session: ${currentSessionId})`);
    } catch (ollamaErr) {
      console.error('[WebUI macOS ARM] Failed to start PSF Ollama:', ollamaErr);
      closeLoadingWindow();
    ...
    if (!ollamaReady) {
      console.warn('[WebUI macOS ARM] ⚠️  Ollama may not be responding yet, but continuing...');
    } else {
      console.log(`[WebUI macOS ARM] ✅ Ollama is responding on port ${ollamaPort}`);
    }
    ...
    const webuiDir = path.join(dirname, '..', 'binaries', 'python-webui');
    const platformDir = 'macos-arm';
    
    console.log('[WebUI macOS ARM] 📦 Platform: macos-arm');
    ...
    if (openWebuiProcess && !openWebuiProcess.killed) {
      console.log('[WebUI macOS ARM] ℹ️  Open WebUI already running');
    ...
    if (!webuiPort) {
      console.error('[WebUI macOS ARM] ❌ Failed to allocate WebUI port');
      closeLoadingWindow();
      return { success: false, message: 'Failed to allocate port for Open WebUI' };
    }
    console.log(`[WebUI macOS ARM] ✅ Allocated WebUI port: ${webuiPort}`);
    
    // Launch Open WebUI
    console.log('[WebUI macOS ARM] 🚀 Launching Open WebUI...');
    ...
    openWebuiProcess.on('exit', (code) => {
      console.log(`[WebUI macOS ARM] ⚠️  Open WebUI exited with code ${code}`);
      openWebuiProcess = null;
    });
    
    openWebuiProcess.on('error', (err) => {
      console.error('[WebUI macOS ARM] ❌ Open WebUI spawn error:', err.message);
    });
    
    console.log('[WebUI macOS ARM] ✅ Open WebUI process started');
    
    // Wait for WebUI to be ready
    console.log('[WebUI macOS ARM] ⏳ Waiting for Open WebUI to start...');
    ...
    for (let i = 0; i < maxWaitSeconds; i++) {
      ...
      if (checkReady) {
        webuiReady = true;
        console.log(`[WebUI macOS ARM] ✅ Open WebUI ready after ${i + 1} seconds`);
        break;
      }
      
      if ((i + 1) % 5 === 0) {
        console.log(`[WebUI macOS ARM] ⏳ Still waiting... (${i + 1}/${maxWaitSeconds})`);
      }
    }
    
    if (!webuiReady) {
      console.error('[WebUI macOS ARM] ❌ Open WebUI did not start in time');
      closeLoadingWindow();
      return { success: false, message: 'Open WebUI failed to start.' };
    }
    
    // Wait for Ollama connection
    console.log('[WebUI macOS ARM] ⏳ Waiting for Open WebUI to connect to Ollama...');
    ...
    for (let i = 0; i < maxOllamaWait; i++) {
      ...
      if (checkOllama) {
        ...
        console.log(`[WebUI macOS ARM] ✅ Open WebUI connected to Ollama after ${i + 1} seconds`);
        break;
      }
      
      if ((i + 1) % 3 === 0) {
        console.log(`[WebUI macOS ARM] ⏳ Still connecting... (${i + 1}/${maxOllamaWait})`);
      }
    }
    
    if (!ollamaConnected) {
      console.warn('[WebUI macOS ARM] ⚠️  May not be fully connected, but opening anyway...');
    }
    ...
    console.log(`[WebUI macOS ARM] Updated session: ${currentSessionId}`);
    
    // =========================================================================
    // BMOC: Simplified cleanup - BMOC handles Ollama via closeSession
    // =========================================================================
    openWebuiWindow.on('closed', () => {
      console.log('[WebUI macOS ARM] 🛑 Window closed, stopping backend...');
      ...
      // Release WebUI port
      if (webuiPort) {
        console.log(`[WebUI macOS ARM] 🛑 Releasing WebUI port ${webuiPort}...`);
    ...
    console.error('[WebUI macOS ARM] ❌ Launch error:', err);
