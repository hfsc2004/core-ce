/**
 * ollama-manager-macos-arm.js
 * macOS Apple Silicon Platform-Specific Implementation
...
/**
 * Kill stale PSF Ollama processes (macOS ARM implementation)
 */
async function killStalePSFOllama(appPath) {
  console.log('[macOS ARM] Checking for stale PSF Ollama processes...');
  
  const binariesPath = path.join(appPath, '..', 'binaries', 'ollama', 'macos-arm', 'bin');
      ...
      try {
        ...
        for (const pid of pids) {
          ...
          if (cmdline.includes(binariesPath)) {
            console.log(`[macOS ARM] Killing stale PSF Ollama process on port ${port} (PID: ${pid})`);
            try {
...
async function startOllamaServerOnPort(appPath, gpuInfo, port) {
  console.log(`[macOS ARM] Starting Ollama server on pre-allocated port ${port}...`);
  ...
  const ollamaPath = pathManager.getOllamaBinaryPath(appPath, 'macos-arm');
  ...
  if (gpuInfo && gpuInfo.accelerationType === 'apple-silicon') {
    console.log(`[macOS ARM] Using Metal acceleration (${gpuInfo.name})`);
  }
  ...
  ollamaProcess.stdout.on('data', (data) => {
    console.log('[macOS ARM] Ollama stdout:', data.toString().trim());
  });
  
  ollamaProcess.stderr.on('data', (data) => {
    console.log('[macOS ARM] Ollama stderr:', data.toString().trim());
  });
  
  console.log(`[macOS ARM] ✅ Ollama ready: PID ${ollamaProcess.pid} on port ${port}`);
...
/**
 * Start PSF Ollama Server (macOS ARM)
...
async function startOllamaServer(appPath, gpuInfo, serviceType = 'terminal') {
  console.log(`[macOS ARM] Starting Ollama server (serviceType: ${serviceType})...`);
  ...
  if (psfOllamaProcess && psfOllamaPort) {
    console.log(`[macOS ARM] PSF Ollama already running on port ${psfOllamaPort}`);
  ...
  switch (normalizedType) {
    ...
    case 'open-webui':
      ...
      console.log(`[macOS ARM] Allocated port ${port} from WEBUI pool`);
      break;
    case 'anythingllm':
    case 'anything-llm':
      port = PortPool.getAnythingLLMPort(`AnythingLLM Ollama [direct]`);
      console.log(`[macOS ARM] Allocated port ${port} from ANYTHINGLLM pool`);
      break;
    case 'terminal':
    default:
      port = PortPool.getTerminalPort(`Terminal Ollama [direct]`);
      console.log(`[macOS ARM] Allocated port ${port} from TERMINAL pool`);
      break;
  }
  ...
  console.log(`[macOS ARM] ✅ PSF Ollama Server started on port ${port} (PID: ${psfOllamaProcess.pid})`);
  
  return port;
}

/**
 * Stop PSF Ollama Server (macOS ARM)
 * Kills entire process group to include runner subprocesses
 */
function stopOllamaServer() {
  if (psfOllamaProcess) {
    console.log('[macOS ARM] Stopping PSF Ollama Server (killing process group)...');
    const pid = psfOllamaProcess.pid;
    try {
      // Kill entire process group (negative PID) - this kills runners too
      process.kill(-pid, 'SIGTERM');
      console.log(`[macOS ARM] Killed process group ${pid}`);
    } catch (err) {
      // Fallback to killing just the process
      try {
...
/**
 * Open Ollama Terminal (macOS ARM)
...
async function openOllamaTerminal(appPath, modelName, preloadPath, terminalHtmlPath, gpuInfo, modelVramMB = 0, collection = '', modelConfig = null) {
  ...
  console.log('[macOS ARM] Opening Ollama Terminal...');
  
  // Use the EXISTING server that launchModelInOllama started
  if (!psfOllamaProcess || !psfOllamaPort) {
    console.error('[macOS ARM] ERROR: No Ollama server running! Model must be loaded first.');
    return { success: false, message: 'No Ollama server running. Load model first.' };
  }
  
  const port = psfOllamaPort;
  console.log(`[macOS ARM] Using existing Ollama server on port ${port}`);
  ...
  if (modelConfig) {
    ...
    console.log(`[macOS ARM] Including model config in terminal URL`);
  }
  terminalWindow.loadURL(url);
  
  // Cleanup on close - kill the MAIN server since terminal is done
  terminalWindow.on('closed', async () => {
    console.log('[macOS ARM] Terminal window closed, closing Ollama session...');
    ...
    if (psfOllamaProcess) {
      ...
      try {
        process.kill(-pid, 'SIGTERM');
        console.log(`[macOS ARM] Killed Ollama process group ${pid}`);
      } catch (err) {
    ...
    console.log('[macOS ARM] Ollama session closed');
  });
...
async function stopAllInstances() {
  ...
  try {
    const { stdout } = await execPromise(`pgrep -f "binaries/ollama/macos-arm"`);
    const pids = stdout.trim().split('\n').filter(p => p);
    
    for (const pid of pids) {
      try {
        console.log(`[macOS ARM] Killing remaining Ollama process PID: ${pid}`);
        process.kill(parseInt(pid), 'SIGTERM');
      } catch (err) {
    ...
    if (pids.length > 0) {
      console.log(`[macOS ARM] Cleaned up ${pids.length} remaining Ollama process(es)`);
    }
  } catch (err) {
    // No matching processes found - that's fine
  }
  
  console.log('[macOS ARM] All Ollama instances stopped');
}
...
module.exports = {
  ...
  stopAllInstances,
  getPSFOllamaPort: () => {
    console.warn('[macOS ARM] ⚠️  DEPRECATED: getPSFOllamaPort() called. Use session-manager.getOllamaPortForService() instead.');
    return psfOllamaPort;
  },
