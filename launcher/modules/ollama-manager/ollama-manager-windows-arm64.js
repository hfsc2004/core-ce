/**
 * ollama-manager-windows-arm64.js
 * Windows ARM64 Platform-Specific Implementation
...
/**
 * Kill stale PSF Ollama processes (Windows ARM64 implementation)
 */
async function killStalePSFOllama(appPath) {
  console.log('[Windows ARM64] Checking for stale PSF Ollama processes...');
  
  const binariesPath = path.join(appPath, '..', 'binaries', 'ollama', 'windows-arm64', 'bin');
        ...
        for (const line of lines) {
          ...
          if (match) {
            ...
            if (pathOutput.includes(binariesPath)) {
              console.log(`[Windows ARM64] Killing stale PSF Ollama process on port ${port} (PID: ${pid})`);
...
async function startOllamaServerOnPort(appPath, gpuInfo, port) {
  console.log(`[Windows ARM64] Starting Ollama server on pre-allocated port ${port}...`);
  ...
  const ollamaPath = pathManager.getOllamaPath(appPath, 'windows-arm64');
  ...
  ollamaProcess.stdout.on('data', (data) => {
    console.log('[Windows ARM64] Ollama stdout:', data.toString().trim());
  });
  
  ollamaProcess.stderr.on('data', (data) => {
    console.log('[Windows ARM64] Ollama stderr:', data.toString().trim());
  });
  
  console.log(`[Windows ARM64] Ollama ready: PID ${ollamaProcess.pid} on port ${port}`);
  
  return { pid: ollamaProcess.pid, port: port, process: ollamaProcess };
}

/**
 * Start PSF Ollama Server (Windows ARM64)
...
async function startOllamaServer(appPath, gpuInfo, serviceType = 'terminal') {
  console.log(`[Windows ARM64] Starting Ollama server (serviceType: ${serviceType})...`);
  
  await killStalePSFOllama(appPath);
  
  if (psfOllamaProcess && psfOllamaPort) {
    console.log(`[Windows ARM64] PSF Ollama already running on port ${psfOllamaPort}`);
  ...
  switch (normalizedType) {
    ...
    case 'openwebui':
      ...
      console.log(`[Windows ARM64] Allocated port ${port} from WEBUI pool`);
      break;
    case 'anythingllm':
      port = PortPool.getAnythingLLMPort('AnythingLLM Ollama [direct]');
      console.log(`[Windows ARM64] Allocated port ${port} from ANYTHINGLLM pool`);
      break;
    case 'terminal':
    default:
      port = PortPool.getTerminalPort('Terminal Ollama [direct]');
      console.log(`[Windows ARM64] Allocated port ${port} from TERMINAL pool`);
      break;
  }
  ...
  psfOllamaSessionId = sessionManager.registerSession({
    ...
    metadata: { gpu: gpuInfo?.name || 'CPU', platform: 'windows-arm64', serviceType }
  });
  ...
  console.log(`[Windows ARM64] PSF Ollama Server started on port ${port} (PID: ${psfOllamaProcess.pid})`);
  
  return port;
}

/**
 * Stop PSF Ollama Server (Windows ARM64)
 * Kills entire process tree to include runner subprocesses
 */
async function stopOllamaServer() {
  if (psfOllamaProcess) {
    console.log('[Windows ARM64] Stopping PSF Ollama Server (killing process tree)...');
    const pid = psfOllamaProcess.pid;
    try {
      // Kill entire process tree with taskkill /T
      await execPromise(`taskkill /F /T /PID ${pid}`);
      console.log(`[Windows ARM64] Killed process tree ${pid}`);
    } catch (err) {
...
/**
 * Open Ollama Terminal (Windows ARM64)
...
async function openOllamaTerminal(appPath, modelName, preloadPath, terminalHtmlPath, gpuInfo, modelVramMB = 0, collection = '', modelConfig = null) {
  ...
  console.log('[Windows ARM64] Opening Ollama Terminal...');
  
  // Use the EXISTING server that launchModelInOllama started
  if (!psfOllamaProcess || !psfOllamaPort) {
    console.error('[Windows ARM64] ERROR: No Ollama server running! Model must be loaded first.');
    return { success: false, message: 'No Ollama server running. Load model first.' };
  }
  
  const port = psfOllamaPort;
  console.log(`[Windows ARM64] Using existing Ollama server on port ${port}`);
  ...
  if (modelConfig) {
    ...
    console.log(`[Windows ARM64] Including model config in terminal URL`);
  }
  terminalWindow.loadURL(url);
  
  // Cleanup on close - kill the MAIN server since terminal is done
  terminalWindow.on('closed', async () => {
    console.log('[Windows ARM64] Terminal window closed, closing Ollama session...');
    ...
    if (psfOllamaProcess) {
      ...
      try {
        await execPromise(`taskkill /F /T /PID ${pid}`);
        console.log(`[Windows ARM64] Killed Ollama process tree ${pid}`);
      } catch (err) {
    ...
    console.log('[Windows ARM64] Ollama session closed');
  });
...
async function stopAllInstances() {
  ...
  try {
    const { stdout } = await execPromise(`wmic process where "ExecutablePath like '%binaries\\\\ollama\\\\windows-arm64%'" get ProcessId`);
    const pids = stdout.match(/\d+/g) || [];
    
    for (const pid of pids) {
      try {
        console.log(`[Windows ARM64] Killing remaining Ollama process PID: ${pid}`);
    ...
    if (pids.length > 0) {
      console.log(`[Windows ARM64] Cleaned up ${pids.length} remaining Ollama process(es)`);
    }
  } catch (err) {
    // No matching processes found - that's fine
  }
  
  console.log('[Windows ARM64] All Ollama instances stopped');
}
...
module.exports = {
  ...
  stopAllInstances,
  getPSFOllamaPort: () => {
    console.warn('[Windows ARM64] DEPRECATED: getPSFOllamaPort() called. Use session-manager.getOllamaPortForService() instead.');
    return psfOllamaPort;
  },
