/**
 * Compile Manager - macOS Intel Implementation
 * 
 * Platform-specific compilation for macOS Intel systems.
 * 
 * Features:
...
/**
 * Compile project for macOS Intel
 * 
...
async function compileProject(fromPath, config, progressCallback = null) {
  try {
    const sendProgress = (status, progress, log = null) => {
      ...
      console.log(`[Compile Manager macOS Intel] [${progress}%] ${status}${log ? ': ' + log : ''}`);
    };
    
    const projectRoot = path.join(fromPath, '..');
    const targetPlatform = 'macos-intel';
    const buildEdition = common.normalizeEdition(config.edition || 'standard');
    
    sendProgress('Starting macOS Intel compilation...', 2, `Target: ${targetPlatform}`);
    ...
    console.error('[Compile Manager macOS Intel] Compile error:', err);
