/**
 * AnythingLLM Manager macOS ARM - Platform-specific implementation
...
const support = require('./anythingllm-manager-macos-arm-support');
...
function spawnAnythingProcesses(anythingPath, env) {
  console.log('[AnythingLLM macOS ARM] Starting backend on port', backendPort);
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      console.log('[AnythingLLM macOS ARM] Starting frontend on port', frontendPort);
...
async function waitForServersReady() {
  console.log('[AnythingLLM macOS ARM] Waiting for servers...');
  console.log(`[AnythingLLM macOS ARM] Checking backend on port ${backendPort}, frontend on port ${frontendPort}`);
  ...
  while (Date.now() - startTime < maxWait) {
    if (!backendReady) {
      ...
      if (backendReady) console.log(`[AnythingLLM macOS ARM] ✅ Backend ready on port ${backendPort}`);
    }
    if (!frontendReady) {
      frontendReady = await common.checkIfServerReady(frontendPort);
      if (frontendReady) console.log(`[AnythingLLM macOS ARM] ✅ Frontend ready on port ${frontendPort}`);
    }
...
function registerAnythingSession() {
  if (!anythingLLMSessionId) {
    ...
    console.log(`[AnythingLLM macOS ARM] Session registered: ${anythingLLMSessionId}`);
    return;
  }
  ...
  console.log(`[AnythingLLM macOS ARM] Session updated: ${anythingLLMSessionId}`);
}
...
async function launchAnythingLLM(__dirname, gpuInfo = null) {
  try {
    ...
    console.log('[AnythingLLM macOS ARM] Starting dedicated Ollama instance...');
    ollamaPort = null;
    ...
    try {
      ...
      console.log(`[AnythingLLM macOS ARM] PSF Ollama started on port ${ollamaPort}`);
      await new Promise((resolve) => setTimeout(resolve, 2000));
    } catch (err) {
    ...
    if (!backendPort || !frontendPort) {
      console.error('[AnythingLLM macOS ARM] Failed to allocate ports from pool');
    ...
    console.log(`[AnythingLLM macOS ARM] Allocated ports - Backend: ${backendPort}, Frontend: ${frontendPort}`);
    ...
    if (!readiness.backendReady || !readiness.frontendReady) {
      console.error(`[AnythingLLM macOS ARM] Timeout waiting for servers. Backend: ${readiness.backendReady}, Frontend: ${readiness.frontendReady}`);
    ...
    console.error('[AnythingLLM macOS ARM] Error:', err);
    await cleanupAnythingLLM();
...
async function cleanupAnythingLLM() {
  console.log('[AnythingLLM macOS ARM] Starting cleanup...');
  ...
  if (backendPort) {
    ...
    console.log(`[AnythingLLM macOS ARM] Released backend port ${backendPort}`);
    backendPort = null;
  }

  if (frontendPort) {
    AnythingLLMPortPool.releasePort(frontendPort);
    console.log(`[AnythingLLM macOS ARM] Released frontend port ${frontendPort}`);
    frontendPort = null;
  }
  ...
  console.log('[AnythingLLM macOS ARM] ✅ Cleanup complete');
}
