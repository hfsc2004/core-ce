...
(function() {
  ...
  function openDryRunPreviewModal(previewText, deps = {}) {
    const escapeHtml = typeof deps.escapeHtml === 'function' ? deps.escapeHtml : ((v) => String(v || ''));
    return new Promise((resolve) => {
      const existing = document.getElementById('moe-chat-dryrun-overlay');
      if (existing) existing.remove();

      const overlay = document.createElement('div');
      overlay.id = 'moe-chat-dryrun-overlay';
      ...
      panel.innerHTML = `
        ...
        <div style="display:flex;justify-content:flex-end;gap:10px;padding:12px;border-top:1px solid rgba(255,255,255,0.12);background:#0f172a;">
          <button id="moe-chat-dryrun-cancel" style="padding:8px 12px;background:rgba(255,255,255,0.08);border:1px solid #666;border-radius:6px;color:#ddd;cursor:pointer;">Cancel</button>
          <button id="moe-chat-dryrun-confirm" style="padding:8px 12px;background:rgba(0,255,136,0.18);border:1px solid #00ff88;border-radius:6px;color:#00ff88;cursor:pointer;font-weight:bold;">Execute Live</button>
        </div>
      `;
      ...
      panel.querySelector('#moe-chat-dryrun-cancel')?.addEventListener('click', () => close(false));
      panel.querySelector('#moe-chat-dryrun-confirm')?.addEventListener('click', () => close(true));
    });
  }

  function formatChatContent(text, options = {}, deps = {}) {
    const escapeHtml = typeof deps.escapeHtml === 'function' ? deps.escapeHtml : ((v) => String(v || ''));
  ...
  function buildRouteTraceLine(meta, deps = {}) {
    const escapeHtml = typeof deps.escapeHtml === 'function' ? deps.escapeHtml : ((v) => String(v || ''));
