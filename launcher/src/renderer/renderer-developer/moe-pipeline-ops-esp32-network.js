...
function getCameraBoardProfileConfig(esp32 = {}) {
  ...
  if (profile === 'elegoo-esp32s3-camera-v1') {
    return {
      ...
      pinProfiles: [
        {
          key: 'esp32s3-eye-style',
          label: 'ESP32S3_EYE-style',
...
void handleHealth() {
  ...
  body += ",\\"networkMode\\":\\"" + String(wifiApFallback ? "ap" : "sta") + "\\"";
  body += ",\\"ip\\":\\"" + String(wifiApFallback ? WiFi.softAPIP().toString() : WiFi.localIP().toString()) + "\\"";
...
bool initCameraWithPins(const CameraPins& pins) {
  ...
  if (err != ESP_OK) {
    // Retry with a conservative no-PSRAM profile; some S3 boards are wired without PSRAM.
...
bool initCamera() {
  ...
  for (int i = 0; i < CAMERA_PIN_PROFILE_COUNT; i++) {
    const CameraPins& pins = CAMERA_PIN_PROFILES[i];
    if (initCameraWithPins(pins)) {
      cameraReady = true;
      activePinProfile = String(pins.key);
      return true;
    }
    esp_camera_deinit();
  }
...
void startFallbackAp() {
  ...
  if (apOk) {
    wifiApFallback = true;
    Serial.println(String("WiFi: AP fallback started SSID=") + FALLBACK_AP_SSID);
...
async function flashGatewayEsp32CameraFirmware(gatewayId) {
  ...
  try {
    ...
    const result = await window.electronAPI.runMoEIrgContract(contract, { irgModeOverride: 'live' });
    if (!result?.success) {
